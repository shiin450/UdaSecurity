...
Your company, **Udasecurity**, has created a home security com.mukhtar.UdaSecurity.application. This com.mukhtar.UdaSecurity.application tracks the status of sensors, monitors camera input, and changes the alarm state of the system based on inputs. Users can arm the system for when they’re home or away as well as disarm the system.

The wild success of your com.mukhtar.UdaSecurity.application means that you have to prepare to scale your software. You’ll need to write unit tests for all of the major operations the com.mukhtar.UdaSecurity.application performs so that you can safely make more changes in the future. You also need to use maven to streamline the build process and automate your tests and code analysis. More urgently, you need to make sure your com.mukhtar.UdaSecurity.application actually does what it’s supposed to do, and so one of writing thorough unit tests will help you find any bugs that already exist.

The image analysis com.mukhtar.UdaSecurity.service used by this com.mukhtar.UdaSecurity.application has proven popular as well, and another team wants to use it in their project. To accomplish this, you must separate the Image Service from the program and package it as a separate module to be included both in your own project and in other projects.

The end goal for this assignment is to split the project into multiple modules, refactor it to be unit-testable, write unit tests to cover all the main requirements for the Security portion of the com.mukhtar.UdaSecurity.application and fix any bugs that you find in the process. You’ll also update the build process to automatically run unit tests, perform static code analysis, and build the code into an executable jar file. 
...
1. Inside of the parent project, create one module for your Security Security and one module for your Image com.mukhtar.UdaSecurity.service. (*Note: Use one parent pom at the top level and one child pom for each module.*)
...
You should also write interfaces to describe the necessary behaviors of your dependencies to make them easier to Mock. We’re already using a SecurityRepository interface, but we have no interface to describe the behavior of our Image Service. Create an interface that makes it easy to test our com.mukhtar.UdaSecurity.application regardless of whether we’re using the `com.mukhtar.UdaSecurity.Services.AwsImageService` or `com.mukhtar.UdaSecurity.Services.FakeImageService`. 

### *Optional Stand Out Task:* Connect Your Project to the AWS Image Recognition Library

> Once you have created an interface for your image com.mukhtar.UdaSecurity.service, complete the steps described in the com.mukhtar.UdaSecurity.Services.AwsImageService to create credentials and provide them in a properties file for your com.mukhtar.UdaSecurity.application. Change the ImageService implementation class in the CatpointGui class to use the com.mukhtar.UdaSecurity.Services.AwsImageService instead of the com.mukhtar.UdaSecurity.Services.FakeImageService. Try submitting different types of images and see what comes back!

![Image of the Gui](gui_1.png)

**While you are writing tests, it's possible you may need to refactor the com.mukhtar.UdaSecurity.application in order to make all of the requirements testable.** For example, parts of the business logic may be contained in the GUI or repository classes. You may have to move this logic into the security com.mukhtar.UdaSecurity.service to be tested.
...
*Reminder*: If you find yourself relying on the behavior of another com.mukhtar.UdaSecurity.service, you may want to consider using a Mock!
...
**Your goal is to provide full coverage of all methods that implement the com.mukhtar.UdaSecurity.application requirements.** You don’t need to test trivial methods like getters or setters, but you do need to make sure that all the lines in your other methods are reachable by the unit tests.
...
> These tests can call com.mukhtar.UdaSecurity.service methods and then use JUnit Assertions to verify that the values you retrieve after performing operations are the expected values.

## Section 6: Build the Application into an Executable JAR
Update your `pom.xml` to use a maven plugin that allows you to compile your com.mukhtar.UdaSecurity.application into an executable JAR. Confirm that you can run the program by running the jar file. Execute the Maven goal that builds the JAR and start the com.mukhtar.UdaSecurity.application from the command line.

Submit a screenshot titled `executable_jar.png` that shows you running the executable jar from the command line and the com.mukhtar.UdaSecurity.application launching. Use the command `java -jar [yourjarname]` to run it.
